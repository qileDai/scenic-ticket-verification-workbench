import type { GroupBuyCoupon, VerificationRecord, EntranceGate, VisitorCredential, EntityStatus } from '../types'
...
export function calculateVerificationSuccessRate(records: VerificationRecord[]): number {
  ...
  const successCount = records.filter((record) => record.isSuccess).length
...
export function calculateDuplicateRate(coupons: GroupBuyCoupon[], records: VerificationRecord[]): number {
  if (records.length === 0 || coupons.length === 0) return 0
...
export function detectDuplicateVerification(records: VerificationRecord[]): Array<{
  ...
  couponRecords.forEach((couponRelatedRecords, couponId) => {
    if (couponRelatedRecords.length > 1) {
      const sortedTimes = couponRelatedRecords.map((record) => record.verificationTime).sort()
      duplicates.push({
        couponId,
        count: couponRelatedRecords.length,
...
export function aggregateMetricsByDate<T>(
  items: T[],
...
export function aggregateMetricsByBatch<T extends { batchId: string }>(
  items: T[],
  valueFn: (item: T) => number
  ...
  items.forEach((item) => {
    const value = valueFn(item)
    batchMap.set(item.batchId, (batchMap.get(item.batchId) || 0) + value)
  })
...
export function aggregateMetricsByOwner<T extends { owner: string }>(
  items: T[],
  valueFn: (item: T) => number
